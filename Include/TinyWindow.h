...
namespace TinyWindow
{
    ...
    public:
        ...
        formatSetting_t(int redBits = 8, int greenBits = 8, int blueBits = 8, int alphaBits = 8,
            ...
            int accumRedBits = 8, int accumGreenBits = 8, int accumBlueBits = 8, int accumAlphaBits = 8,
    ...
    struct windowSetting_t
    {
        ...
        windowSetting_t(const char* name = nullptr, void* userData = nullptr,
            ...
            int versionMajor = 4, int versionMinor = 2, unsigned int colorBits = 8, unsigned int depthBits = 24, unsigned int stencilBits = 8, unsigned int accumBits = 8,
            state_t currentState = state_t::normal, profile_t profile = profile_t::compatibility)
        {
            this->name = name;
            this->resolution = resolution;
    ...
    private:
        ...
        void InitializePixelFormat(tWindow* window)
        {
            ...
            else
            {
                ...
                formatSetting_t* desiredSetting = new formatSetting_t(window->settings.colorBits, window->settings.colorBits, window->settings.colorBits, window->settings.colorBits,
                window->settings.depthBits, window->settings.stencilBits, window->settings.accumBits, window->settings.accumBits, window->settings.accumBits, window->settings.accumBits);
...
#if defined(DEBUG)
					printf("matching FBConfig %i, visual ID 0x%2x: samplebuffers = %i, samples = %i\n",
						   configIndex, visualInfo->visualid, sampleBuffer, samples);

					printf("FBConfig: \nID: %i, bufferSize: %i, doubleBuffer?: %i, numAuxBuffers: %i, redSize: %i, greenSize: %i, blueSize: %i, alphaSize: %i, depthSize: %i, stencilSize: %i \n\n",
						   configIndex, bufferSize, doubleBuffer, numAuxBuffers, redSize, greenSize, blueSize, alphaSize, depthSize, stencilSize);
#endif//DEBUG
        ...
        std::error_code Linux_InitGL(tWindow* window)
        {
            ...
            if (window->context)
            {
                ...
                XGetWindowAttributes(currentDisplay,
                    window->windowHandle, &attributes);
